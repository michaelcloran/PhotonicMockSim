...
import com.photoniccomputer.photonicmocksim.dialogs.KeyboardDialog;
...
public class ChooseKeyboardHubDialog extends JDialog{
    public ChooseKeyboardHubDialog(PhotonicMockSim theApp){
        this.theApp = theApp;
            ...
            for(Layer layer : part.getLayersMap().values()){
                for(Module module: layer.getModulesMap().values()){
                    for(CircuitComponent component : module.getComponentsMap().values()){
                        if(component.getComponentType() == KEYBOARDHUB){
                        ...
                        public void actionPerformed(ActionEvent e) {
                            if(partComboBox.getItemCount()!= 0){
                                ...
                                if(selectedPartNumber != 0 && selectedLayereNumber != 0 && selectedModuleNumber != 0 && selectedComponentNumber!= 0){
                                    setVisible(false);
                                    dispose(); 
                                    new KeyboardDialog(theApp, selectedPartNumber, selectedLayereNumber, selectedModuleNumber, selectedComponentNumber);
                                }
                            } 
                        }
                }
