...
public class FontDialog extends JDialog implements ActionListener, ListSelectionListener, ChangeListener{
    public FontDialog(JFrame window){
        ...
        sizePane.add(label);
        chooseSize = new JSpinner(new SpinnerNumberModel(font.getSize(), POINT_SIZE_MIN, POINT_SIZE_MAX, POINT_SIZE_STEP));
    ...
    public void actionPerformed(ActionEvent e){
        if(e.getSource() == ok){
            ((JFrame)getOwner()).setFont(font);
        }else{
            font = ((JFrame)getOwner()).getFont();
            fontDisplay.setFont(font);
            chooseSize.setValue(font.getSize());
            fontList.setSelectedValue(font.getName(),true);
            int style = font.getStyle();
            bold.setSelected((style & Font.ITALIC) > 0);
        }
        setVisible(false);
    }
